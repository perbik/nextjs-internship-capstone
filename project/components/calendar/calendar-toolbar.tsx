	...
	return (
		<header className="flex flex-col gap-4 border-b border-border px-5 py-4 lg:flex-row lg:items-center lg:justify-between lg:px-6 ">
			...
			<div className="flex flex-wrap items-center gap-1">
				<div className="mr-1">{addEventAction}</div>
				<nav aria-label="Calendar view" className="flex items-center gap-1">
					{views.map((option) => (
						<Button
							...
							size="sm"
							asChild
							className={`rounded-lg px-3 font-semibold ${view === option.value ? "bg-brand text-white hover:bg-brand/90 hover:text-white" : "text-muted-foreground hover:bg-brand/5 hover:text-brand"}`}
						>
							<Link
